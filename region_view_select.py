...
def paint_object_pictures(bboxes, object_ids, object_types, visible_view_object_dict, extrinsics_c2w, axis_align_matrix,
    ...
    _paint_object_pictures(bboxes, object_ids, object_types, visible_object_view_dict, extrinsics_c2w, view_ids, intrinsics, depth_intrinsics, depth_map_paths, color_map, image_dir, output_dir,
                           output_type=output_type)


def paint_group_pictures(group_dict, bboxes, object_ids, object_types, visible_view_object_dict, extrinsics_c2w, axis_align_matrix, intrinsics, image_paths, blurry_image_ids_path, output_dir, need_not_to_draw_list=['wall']):
...
def get_best_view(o3d_bbox, extrinsics_c2w, depth_intrinsics, depth_maps, show=False):
    ...
    for i in range(len(extrinsics_c2w)):
        ...
        # now consider for centerness
        # shape (4,)
        center_pos = depth_intrinsic @ np.linalg.inv(
            extrinsic_c2w) @ box_center
        cx, cy = center_pos[:2] / center_pos[2]
...
def _paint_object_pictures(bboxes, object_ids, object_types, visible_object_view_dict, extrinsics_c2w, view_ids,
    ...
    assert output_type in [
        "paint", "crop"], "unsupported output type {}".format(output_type)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    if skip_existing:
        files_and_folders = os.listdir(output_dir)
    ...
    for i in pbar:
        ...
        best_view_index = get_best_view(
            bbox, selected_extrinsics_c2w, selected_depth_intrinsics, selected_depth_maps, show=(object_id == 150))
...
if __name__ == '__main__':
    ...
    # The result of manual annotation
    input_annotation_path = f"example_data/anno_lang/region_anno/{scene_name}.png.txt"
    ...
    if os.path.exists("example_data/example.npy"):
        ...
        anno = read_annotation_pickle(
            'example_data/embodiedscan_infos_train_full.pkl')['scene0000_00']
    ...
    real_image_paths = ['example_data/posed_images'+path[-10:]
                        for path in image_paths]

    # map 2d to 3d
    point_cloud_path = "example_data/lidar/main.pcd"
    min_x, min_y, region_with_label = get_data(
        input_annotation_path, point_cloud_path)
    region_with_label = process_data(
        region_with_label, object_ids, bboxes, min_x, min_y)
    ...
    paint_group_pictures(group_dict, bboxes, object_ids, object_types, visible_view_object_dict,
                         extrinsics_c2w, axis_align_matrix, intrinsics, real_image_paths, blurry_image_ids_path, output_dir)
