...
with gr.Blocks() as demo:
    ...
    total_vertex_num = gr.Slider(label="Vertex Number", info="How different corners can be in a segment.", minimum=3, maximum=10, step=1, value=4)
        ...
        if not poly_done:
            ...
            if len(click_evt_list) == vertex_num:
                ...
                for object_id in object_ids:
                    ...
                    #global vertex_list
                    if object_types[k] not in exclude_type and is_in_poly((o_x,o_y),vertex_list):

                        new_out[max(o_x - 1, 0):min(o_x + 1, out.shape[0] - 1),
                        max(o_y - 1, 0):min(o_y + 1, out.shape[1] - 1)] = np.array(
